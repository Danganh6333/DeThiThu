...
const UpdateModal = ({visible, item, onClose}) => {
  ...
  const [giaBan, setGiaBan] = useState(
    item ? item.gia_ban_ph33497.toString() : '',
  );
  ...
  const chooseImage = useCallback(() => {
    ...
    ImagePicker.launchImageLibrary(options, response => {
      if (response.didCancel) {
        ...
        setHinhAnh(`data:image/jpeg;base64,${response.asset.base64}`);
      }
    });
  }, []);
  ...
  const handleUpdate = () => {
    ...
    dispatch(updateXeMayApi({id: id, data: updatedItem}))
      .then(result => {
